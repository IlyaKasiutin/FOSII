...
from .layers.rnn import RNN as RNNBlock
import tqdm


class SequenceRNN(Module):
    def __init__(self, n_inputs: int, n_hidden: int, n_outputs: int,
                 time_steps: int = 5):
        """
        Simple RNN model that unrolls the given RNN block for a fixed number
        of steps (default 5). Follows project patterns from cnn.py and mlp.py.

        Note: The provided RNN block uses matrices U, W, V with the same
        ...
        # Use the provided single-step RNN block
        # The block internally maintains its state via prev_s
        self.block = RNNBlock(n_inputs=n_inputs, n_hidden=n_hidden, n_outputs=n_outputs)
        ...
        self._modules = {"rnn_block": self.block}

        # Caches for BPTT
        self._cache: Dict[str, list] = {
            "xs": [],        # list of (n_in, batch)
            "prev_ss": [],   # list of states before each step
            "ys": []         # list of outputs (n_out, batch)
        }

    def _reset_state(self, batch_size: int) -> None:
        # Reset the block's recurrent state to zeros for a new sequence
        # Match the existing storage in the block
        self.block.prev_s = np.zeros_like(self.block.W)
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        Args:
            x: Input of shape (batch, time_steps, n_inputs)

        Returns:
            y: Output logits of shape (batch, n_outputs) from the last step
        """
        # batch_size, time_len, _ = x.shape

        # Clear caches
        self._cache["xs"].clear()
        self._cache["prev_ss"].clear()
        ...
        for t in range(self.time_steps):
            # Prepare step input as (n_in, batch)
            # x_t = x[:, t, :].T
            # x_t = x[t, :].reshape(1, -1).T
            x_t = x[t, :]
            # print(f"{x_t.shape=}")

            # Cache state before step for BPTT
            prev_s_t = self.block.prev_s.copy()

            y_t = self.block.single_step(x_t)  # (n_out, batch)

            # Update caches
            self._cache["xs"].append(x_t)
            self._cache["prev_ss"].append(prev_s_t)
            self._cache["ys"].append(y_t)

            # For next step, the block keeps its internal state
            y_last = y_t

        # Return last step output as (batch, n_out)
        return y_last.T
    ...
    def backward(self, loss_grad: np.ndarray) -> None:
        ...
        Args:
            loss_grad: dL/dy_last of shape (batch, n_outputs)
        """
        # Initialize accumulated grads for the block's parameters
        dU_total = np.zeros_like(self.block.U)
        dW_total = np.zeros_like(self.block.W)
        dV_total = np.zeros_like(self.block.V)

        # diff_s is gradient flowing into state from future time steps
        diff_s = np.zeros_like(self.n_hidden)  # match block's prev_s shape usage

        # Only the last step receives direct gradient from the loss in many-to-one
        dmulv_next: Optional[np.ndarray] = None

        for t in reversed(range(self.time_steps)):
            x_t = self._cache["xs"][t]
            prev_s_t = self._cache["prev_ss"][t]

            if t == self.time_steps - 1:
                # loss_grad is (batch, n_out) -> transpose to (n_out, batch)
                dmulv = loss_grad.T
            else:
                # No direct loss on earlier outputs in many-to-one
                dmulv = np.zeros_like(self._cache["ys"][t])

            dprev_s, dU, dW, dV = self.block.backward(x_t, prev_s_t, diff_s, dmulv)
        ...
        # Stash grads on the block and update here to avoid relying on the
        # block's update_params (which references undefined bias fields)
        self.block.U_grad = dU_total
        self.block.W_grad = dW_total
    ...
    def update_params(self, learning_rate: float = 0.001, optimizer=None) -> None:
        ...
        if hasattr(self.block, "U_grad"):
            self.block.U -= learning_rate * self.block.U_grad
        if hasattr(self.block, "W_grad"):
            self.block.W -= learning_rate * self.block.W_grad
        if hasattr(self.block, "V_grad"):
            self.block.V -= learning_rate * self.block.V_grad

    def zero_grad(self) -> None:
        if hasattr(self.block, "U_grad"):
            self.block.U_grad.fill(0)
        if hasattr(self.block, "W_grad"):
            self.block.W_grad.fill(0)
        if hasattr(self.block, "V_grad"):
    ...
    def train_step(self, X: np.ndarray, y_true: np.ndarray, loss_fn,
        ...
        Args:
            X: (batch, time_steps, n_inputs)
            y_true: (batch, n_outputs) one-hot for CE or targets for regression
        """

        y_pred = self.forward(X)  # (batch, n_outputs)

        # Align with patterns in cnn/mlp losses
        loss_vals = loss_fn.forward(y_pred.T, y_true.T)
        batch_loss = float(np.mean(loss_vals))

        loss_grad = loss_fn.backward(y_pred.T, y_true.T).T  # (batch, n_outputs)
